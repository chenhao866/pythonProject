...
#向数据库插入数据
def setSql(urlpath,urltype,time,urlclass):
    conn = pymysql.connect(host='localhost',port=3306,db='test',user='root',passwd='123456',charset='utf8')
    cur = conn.cursor()
    try:
        sql = "insert into tb_godimgurl(url,urltype,creatorTime,urlclass) values('{0}','{1}','{2}','{3}')".format(urlpath,urltype,time,urlclass)
        cur.execute(sql)
        conn.commit()
    except:
        conn.rollback()
